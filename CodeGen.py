...
# Define available CodeGen models
CODEGEN_MODELS = {
...
# Default model
DEFAULT_MODEL = "codegen-350M-mono"
...
Choose from different model sizes, with larger models generally providing better results but taking longer to process.
You can also edit the generated code in the built-in editor and save your work.
""")
...
with st.sidebar:
    ...
    with settings_tab:
        ...
        selected_model = st.selectbox(
            ...
            format_func=lambda x: f"{x} ({x.split('-')[1]})",
            index=list(CODEGEN_MODELS.keys()).index(st.session_state.selected_model),
            help="Select the CodeGen model to use. Larger models may produce better results but take longer to generate."
        )
